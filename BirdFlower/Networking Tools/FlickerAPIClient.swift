...
struct FlickerAPI {
    static func searchPhotos(searchQuery: String, contentType: ListType, completion: @escaping (Result<FlickerSearchResult, AppError>) -> ()) {
        ...
        let safeFlickerPhotoSearchEndpoint = "https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=\(Secrets.flickerAPIKey)&tags=\(search ?? "Taco")&per_page=1&page=&format=json&nojsoncallback=1&safe_search=1".lowercased()
        ...
        guard let url = URL(string: safeFlickerPhotoSearchEndpoint) else {
            completion(.failure(.badURL(safeFlickerPhotoSearchEndpoint)))
            return
        }
